        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 flex-1">
          {PLANS.map((plan) => (
            <div key={plan.name} className={`pricing-card flex flex-col p-8 ${plan.popular ? 'popular relative scale-[1.02] shadow-xl shadow-indigo-500/5' : 'shadow-sm border border-slate-50'}`}>
              ...
              <button
                className={`w-full py-3 rounded-xl text-[13px] font-medium transition-all shrink-0 ${plan.popular
                    ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/10 hover:opacity-90'
                    : plan.name === 'Free'
